...
def get_all_events(_events):
    ...
    c_event = None
    #
    _bes = _events.find("BEGIN:VEVENT")
    while _bes != -1:
        _bee = _events.find('END:VEVENT', _bes)
        list_events.append(_events[_bes+13:_bee-1])
        _bes = _bes = _events.find("BEGIN:VEVENT", _bee)
    #
    return list_events
